...
function ($scope, NgTableParams, $timeout, $parse, $compile, $attrs, $element, ngTableColumn) {
    ...
    angular.module('ngTable').run(['$templateCache', function ($templateCache) {
        ...
        $templateCache.put('ng-table/filters/text.html', '<input type="text" name="{{name}}" style="max-width:200px" placeholder="Search" ng-disabled="$filterRow.disabled" ng-model="params.filter()[name]" ng-if="filter==\'text\'" class="input-filter form-control"/>');
        $templateCache.put('ng-table/filters/searchbook.html', '<input type="text" name="{{name}}"  ng-disabled="$filterRow.disabled" ng-model="params.filter()[name]" ng-if="filter==\'text\'" class="input-filter form-control"/>');
