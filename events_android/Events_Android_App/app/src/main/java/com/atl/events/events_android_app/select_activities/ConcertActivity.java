...
public class ConcertActivity extends AppCompatActivity{
    ...
    private void select_from_concert(String id) {
        eventsapi.selectConcert(new select_domain(id), new Callback<Concert>() {
            @Override
            public void success(Concert concert, Response response) {
                Picasso.with(getApplication())
                        .load(Webservice_urls.image_url + concert.getLOGO_NAME())
                        .into(logo);
            ...
            public void failure(RetrofitError error) {
                Toast.makeText(ConcertActivity.this, "Something went wrong, Please try again.", Toast.LENGTH_LONG).show();
            }
        });
