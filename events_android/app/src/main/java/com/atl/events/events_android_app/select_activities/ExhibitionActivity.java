...
public class ExhibitionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.select_exhibition_layout);
    ...
    private void select_from_exhibition(String id) {
        new  AsyncTask<String, Void, Exhibition>() {
            @Override
            ...
            protected Exhibition doInBackground(String... params) {
                return Request.getData().selectExhibition(params[0]);
            }

            @Override
            protected void onPostExecute(final Exhibition exhibition) {
                super.onPostExecute(exhibition);
                Picasso.with(getApplication())
                        .load(Request.image_url + exhibition.getLOGO_NAME())
                        .into(logo);
                tv_description.setText(exhibition.getDESCRIPTION());
                tv_start_date.setText(exhibition.getSTART_DATE());
                tv_end_date.setText(exhibition.getEND_DATE());
                tv_price.setText(exhibition.getPRICE() + " " + exhibition.getPRICE_COST()+ " AZN");
                tv_address.setText(exhibition.getADDRESS());
                tv_author.setText(exhibition.getAUTHOR());
                collapsingToolbarLayout.setTitle(exhibition.getTITLE());

                final LatLng position = new LatLng(exhibition.getLATITUDE(), exhibition.getLONGITUDE());
                ...
                map.setOnMapClickListener(new GoogleMap.OnMapClickListener() {
                    ...
                    public void onMapClick(LatLng latLng) {
                        ...
                        i.putExtra("lat", String.valueOf(exhibition.getLATITUDE()));
                        i.putExtra("lon", String.valueOf(exhibition.getLONGITUDE()));
                        i.putExtra("adress", exhibition.getADDRESS());
                        startActivity(i);
                    }
                });
